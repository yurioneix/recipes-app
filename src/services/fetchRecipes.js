...
export const fetchFoodsOrDrinks = async (query) => {
  const limit = 12;
  try {
    const request = await fetchRecipes(`https://www.the${query}db.com/api/json/v1/1/search.php?s=`);
...
export const fetchCategories = async (query) => {
  const limit = 5;
  try {
    const request = await fetch(`https://www.the${query}db.com/api/json/v1/1/list.php?c=list`);
    const response = await request.json();
    const result = (response.meals || response.drinks)
